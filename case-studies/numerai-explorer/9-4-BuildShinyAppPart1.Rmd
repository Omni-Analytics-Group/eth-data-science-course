...
First, download the dataforshiny folder containing 5 csv.files. These data are obtained from the Numerai Tournament training data. 
...
We will first build an app for each dataset in the folder. In other words, we will build 5 small apps. 

After that, we will combine these apps into one!


## The summary data 

We have seen and used this dataset in the previous section. Let's start with the existing code in Example_01.R and add another functionality on there. 

First, we can change the names of the columns so that it is more readable as follows.
...
Let's create a <b> checkboxInput </b> to let the user pick between two plots. 
...
You can download the code part1.R and make sure it is in the same directory of the folder dataforshiny. 
