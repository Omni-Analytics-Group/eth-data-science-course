...
Next, for those individual accounts, we browsed to their page and scraped all the addresses they have interacted with. Then we used those final contracts and scraped this info for each of those if it's a contract and skipped if an address.

Ultimately, the data is people who have interacted with the Unlock contract and have interacted with which other contracts.

![](final.png)

## Goals

Using the scraped data, we wish to do some of the following:

- Clean and process the data to make it suitable for analysis, as needed
...
Now let's compute the top contract tags:
...
We can use nearly the same routine to look at the top contract names:
...
The story is much the same, with Uniswap at the top, followed by InBetweeners and FiatTokenProxy.
...
We hope you enjoyed this brief look into the Unlock Protocol and analyzing data on transactions associated with it from the blockchain. Be on the lookout for many more case studies coming down the pipeline!
