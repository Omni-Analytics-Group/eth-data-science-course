...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, cache = TRUE, tidy = FALSE )
...
To be suitable for analysis, this data required some minor data manipulation of its fields. We will utilize the <b> stringi </b> package to assist with string processing. Here is what we do:

1. Remove 'X1' using <b> select(-X1) </b>
...
Let's take a look at the data after all the manipulation we did.
...
.center[![summary1_1](summary1_1.PNG)]
...
.center[![summary1_2](summary1_2.PNG)]
...
---
We can plot a scatterplot using <b> geom_point </b> from the <b> ggplot2 </b> package to see how the number changes over time. We will exclude activationEpoch 0 using <b> filter() </b> function for the purpose of getting a better graph. 
...
---
Let's take a look at the time series of its cumulative number over time by using the <b> cumsum() </b> and the <b> geom_line() </b> functions. 
...
```
The histogram looks great! We can dive in to the table to see exactly how many validators exited as well as other summary statistics. We will simply use the same formula as mentioned and filter out the rows where 'timeToExit' is empty using <b> is.na() </b>. ]
...
2. Collapse the subset into a long form with two columns: 'variable' and 'value' using <b> gather() </b>
3. Group by the variable and value using <b> group_by </b>
4. Add <b> facet_wrap(~variable) </b> to create plot for each variable
...
One thing to note is that since there is a space in our variable name, we need to use single quote when creating this temporary variable. Also, we can use <b> function(.) scales::percent(., accuracy = 1) </b> to show percentage instead of decimals.
]
...
2. Modify 'exitEpoch' so that it shows 15579 instead of 'NA'. Note that 15579 is the latest epoch when the data was obtained. We can do so by using <b> ifelse() </b>
...
2. Collapse the data into a long form with three columns: 'publickey', 'Variable' and 'Rank' using <b> gather() </b>
3. Create variable 'Score' that give the sum of the rank group by 'publickey' using <b> group_by </b> and <b> sum() </b> 
4. Replace 'Rank' with the rank of the Score (higher the score translate to lower rank) using <b> rank() </b>
5. Sort the table according the 'Rank' using <b> arrange() </b>
...
Note that we divided 'active_time' by 4 to reduce the weighting as we do not want to punish new validators too much.
...
Now that we have their 'Score' and 'Rank', we will put add the them back to the 'valid_stats' dataframe. To do so, we simply use <b> left_join() </b>. Also, we revert 'slashed' and 'skips' to its original value before the negative weighting. 
...
Recreate the Sorted Validator Scores plot. However, this time make sure that the color will show different tiers. An example output is shown below.
...
To create this plot, we use <b> geom_rect() </b> to create those color blocks. For these color blocks to fill the graph beautifully, we first need to modify the valid_tiers dataframe as follows.

1. Subset the 'Tier' and 'Score' columns using <b> select() </b>
2. Add a row that where 'Tier' = 7 and 'Score' = $-\infty$ using <b> rbind() </b>
...
We can present the table using <b> formattable </b> package.
...
We can also add color to each column to show how it changes across different Tiers like a heatmap. Here, we simply use <b> color_tile() </b> that takes two colors as input. 
...
```{r p17, eval=FALSE}
valid_tiers %>%
  mutate(Label = ifelse(Rank %in% c( 80357, 78634, 21108, 62163, 71158, 15372, 4979, 304, 16418, 65418, 74210, 50116,39220), index, NA)) %>%
...
representative <- valid_tiers %>%
  mutate(Label = ifelse(Rank %in% c(55558, 80357, 78634, 21108, 63221, 71158, 15372, 4979, 304, 16418, 46194, 65418, 74210), Rank, NA)) %>%
  filter(!is.na(Label)) %>% 
