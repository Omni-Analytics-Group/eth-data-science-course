...
There are `r nrow(data_new)` observations, or adults, and `r ncol(data_new)` characteristics collected for each adult
...
Notice that the owns_ethereum column is shown as `<chr>` (character). We will convert it to a factor variable.

---

### Assigning data types

The columns which have categorical data are not represented well by the 'character' data type. In R we use the 'factor' data type to represent these columns correctly for modeling.  
...
```{r}
ggplot(data_new, aes(y = capital_gain)) +
  geom_boxplot()
```
...
cor(data %>% select(where(is.numeric), donated, owns_ethereum) %>%
        mutate(owns_ethereum = as.numeric(owns_ethereum), donated = as.numeric(donated)) ,
    use = "complete.obs")
...
What is the percentage of Male vs Female ethereum owners?
...
is this clear enough to answer the question?
---
